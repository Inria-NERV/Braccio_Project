...
def Extract_Data_to_compare(Path_Mi,Path_Rest,time,Trials,N_electrodes,Bins,N_window,Overlap):
    #power spectrum reading
    file_cond_right = load_csv_cond(Path_Mi)
    file_cond_left = load_csv_cond(Path_Rest)

    mat_left = file_cond_left.to_numpy()
    Number_of_trials = Trials
    Number_of_electrodes = N_electrodes
    Number_of_Bins = Bins
    n_window = (N_window/500)
    overlap = Overlap/500
    length = round((time-n_window)/overlap)
    time_leng = np.arange(0,time,overlap)
    mat_left = mat_left[:,2:]
    ...
    mat_right = file_cond_right.to_numpy()
