...
#     return np.sum(np.abs(eigenvalues1 - eigenvalues2))
...
def mahalanobis_distance(matrix1, matrix2):
    covariance = np.cov(matrix1.T)
    inverse_covariance = np.linalg.inv(covariance)
    mean_difference = np.mean(matrix1, axis=0) - np.mean(matrix2, axis=0)
    distance = np.sqrt(np.dot(mean_difference.T, np.dot(inverse_covariance, mean_difference)))
    return distance
...
#############################@
    ...
    Rsigned = (Power_Right_1[:,:,:].mean(0)-Power_Left_1[:,:,:].mean(0))/Power_Left_1[:,:,:].mean(0)


    Rsigned_test = (Power_Right_1_Test[:,:,:].mean(0)-Power_Left_Test_1[:,:,:].mean(0))/Power_Left_Test_1[:,:,:].mean(0)


    Rsigned_test_2 = (Power_Right_2_Test[:,:,:].mean(0)-Power_Left_Test_2[:,:,:].mean(0))/Power_Left_Test_2[:,:,:].mean(0)

    Rsigned_for_Cor = Rsigned[:,7:35]
    Rsigned_test_for_Cor = Rsigned_test[:,7:35]
    Rsigned_test_2_for_Cor = Rsigned_test_2[:,7:35]

    frobenius_norm_score = frobenius_norm(Rsigned_for_Cor, Rsigned_test_for_Cor)
    # trace_distance_score = trace_distance(Rsigned_for_Cor, Rsigned_test_for_Cor)
    spectral_norm_score = spectral_norm(Rsigned_for_Cor, Rsigned_test_for_Cor)
    mahalanobis_distance_score = mahalanobis_distance(Rsigned_for_Cor, Rsigned_test_for_Cor)


    Comp_Cali_vs_Drive1 = [frobenius_norm_score,spectral_norm_score,mahalanobis_distance_score]

    frobenius_norm_score = frobenius_norm(Rsigned_test_for_Cor, Rsigned_test_2_for_Cor)
    # trace_distance_score = trace_distance(Rsigned_test_for_Cor, Rsigned_test_2_for_Cor)
    spectral_norm_score = spectral_norm(Rsigned_test_for_Cor, Rsigned_test_2_for_Cor)
    mahalanobis_distance_score = mahalanobis_distance(Rsigned_test_for_Cor, Rsigned_test_2_for_Cor)

    Comp_Drive1_vs_Drive2 = [frobenius_norm_score,spectral_norm_score,mahalanobis_distance_score]

    frobenius_norm_score = frobenius_norm(Rsigned_for_Cor, Rsigned_test_2_for_Cor)
    # trace_distance_score = trace_distance(Rsigned_for_Cor, Rsigned_test_2_for_Cor)
    spectral_norm_score = spectral_norm(Rsigned_for_Cor, Rsigned_test_2_for_Cor)
    mahalanobis_distance_score = mahalanobis_distance(Rsigned_for_Cor, Rsigned_test_2_for_Cor)

    Comp_Cali_vs_Drive2 = [frobenius_norm_score,spectral_norm_score,mahalanobis_distance_score]


    Summary_Camparison = np.array([Comp_Cali_vs_Drive1,Comp_Drive1_vs_Drive2,Comp_Cali_vs_Drive2])

    return Summary_Camparison
...
a = launch(path, filename_1, filename_2,filename_3, filename_4, filename_5,filename_6, filename_7,filename_8,
                                            ...
                                            True,'1',name_sub)
Score_comp.append(a)
np.save("Correlation_betweenPhases.npy",np.array(Score_comp))
