...
def Power_burg_calculation(Epoch_compute,noverlap,N_FFT,f_max, n_per_seg,filter_order):
    ...
    fs = 500
    nper_segSamples = int(fs * n_per_seg)
    shiftSamples = int(fs * (n_per_seg - noverlap))
    noverlapSamples = nper_segSamples - shiftSamples

    a = Epoch_compute.shape
    M = a[2]  # M = trial length, in samples
    L = nper_segSamples  # L = windowing size, in samples
    noverlap = noverlapSamples  # size of overlapping segment btw windows, in samples
    ...
    return trialspectrum[:,:,round(f_max/(2*fres)):round(f_max/fres)]
...
def launch(path,filename_1,filename_2,filename_3,filename_4,filename_5,filename_6,filename_7,filename_8,cond1,cond2,tmin,tmax,nfft,noverlap,nper_seg,fs,filter_order,number_electrodes,car_bool,strat,name_sub):
    ...
    event_id = {'event{}'.format(i): i for i in range(n_events)}  # Event IDs
    events = np.column_stack((np.arange(n_events), np.zeros(n_events, dtype=int), np.arange(n_events)))
    epochs_MI_Test = mne.EpochsArray(Sig_MI_Reorder_Test[:,:,:], info, events, tmin, event_id)
    #mne.set_eeg_reference(epochs_MI_Test, ref_channels='average', projection=True)
    epochs_Rest_Test = mne.EpochsArray(Sig_Rest_Reorder_Test[:,:,:], info, events, tmin, event_id)
    #mne.set_eeg_reference(epochs_Rest_Test, ref_channels='average', projection=True)
    ...
    epochs_MI_Test_2 = mne.EpochsArray(Sig_MI_Reorder_Test_2, info, events, tmin, event_id)
    #mne.set_eeg_reference(epochs_MI_Test_2, ref_channels='average', projection=True)
    epochs_Rest_Test_2 = mne.EpochsArray(Sig_Rest_Reorder_Test_2, info, events, tmin, event_id)
    #mne.set_eeg_reference(epochs_Rest_Test_2, ref_channels='average', projection=True)
    ...
    freqs = np.arange(13, 25, 1)
    # Define wMNE parameters
    alpha_mi = 55  # Regularization parameter for MI
    alpha_rest = 55  # Regularization parameter for Rest
    loose = 0.2  # Loose orientation constraint


    csd = csd_multitaper(epochs_MI,fmin=13, fmax=25)
    csd_rest = csd_multitaper(epochs_Rest,fmin=13, fmax=25)
    csd_baseline = csd_multitaper(epoch_noise,fmin=13, fmax=25)
    csd_rest = csd_rest.mean()
    csd = csd.mean()
    ...
    baseline_source_power, freqs = apply_dics_csd(csd_rest, filters)
    beta_source_power, freqs = apply_dics_csd(csd, filters)

    stc = (beta_source_power - baseline_source_power)/baseline_source_power
    stc.save(fname=path+name_sub + 'Cali_beta',overwrite=True)
    ...
    #     colormap = 'bwr',
    # )
    # brain.save_image(filename = path+name_sub+'Cali.jpeg')

    csd = csd_multitaper(epochs_MI_Test,fmin=13, fmax=25)
    csd_rest = csd_multitaper(epochs_Rest_Test,fmin=13, fmax=25)
    csd_baseline = csd_multitaper(epoch_noise_Test,fmin=13, fmax=25)
    csd_rest = csd_rest.mean()
    csd = csd.mean()
    csd_baseline = csd_baseline.mean()
    filters = make_dics(
    ...
    stc = (beta_source_power - baseline_source_power) / baseline_source_power
    stc.save(fname=path+name_sub + 'DR1_beta',overwrite=True)
    # message = "DICS source power in the 12-30 Hz frequency band"
    # brain = stc.plot(
    #     hemi="both",
    #     views="axial",
    ...
    #     colormap='bwr',
    # )
    # brain.save_image(filename=path+name_sub + 'DR1.jpeg')

    csd = csd_multitaper(epochs_MI_Test_2,fmin=13, fmax=25)
    csd_rest = csd_multitaper(epochs_Rest_Test_2,fmin=13, fmax=25)
    csd_baseline = csd_multitaper(epoch_noise_Test_2,fmin=13, fmax=25)
    csd_rest = csd_rest.mean()
    csd = csd.mean()
    csd_baseline = csd_baseline.mean()
    filters = make_dics(
    ...
    stc = (beta_source_power - baseline_source_power) / baseline_source_power
    stc.save(fname=path+name_sub + 'DR2_beta',overwrite=True)
    ...
    brain.save_image(filename=path+name_sub + 'DR2.jpeg')
    # Save the list of source estimates
