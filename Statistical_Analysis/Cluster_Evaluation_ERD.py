...
#Essai 9 is good
## Loading
directory_path_Strat_1_cali = '/Users/tristan.venot/Desktop/Experimentation_dataset/Batch2/PSD/PSD_04/_1_/Cali/*'
directory_path_Strat_2_cali = '/Users/tristan.venot/Desktop/Experimentation_dataset/Batch2/PSD/PSD_04/_2_/Cali/*'
directory_path_Strat_3_cali = '/Users/tristan.venot/Desktop/Experimentation_dataset/Batch2/PSD/PSD_04/_3_/Cali/*'


directory_path_Strat_1_drive1 = '/Users/tristan.venot/Desktop/Experimentation_dataset/Batch2/PSD/PSD_04/_1_/Drive1/*'
directory_path_Strat_2_drive1 = '/Users/tristan.venot/Desktop/Experimentation_dataset/Batch2/PSD/PSD_04/_2_/Drive1/*'
directory_path_Strat_3_drive1 = '/Users/tristan.venot/Desktop/Experimentation_dataset/Batch2/PSD/PSD_04/_3_/Drive1/*'


directory_path_Strat_1_drive2 = '/Users/tristan.venot/Desktop/Experimentation_dataset/Batch2/PSD/PSD_04/_1_/Drive2/*'
directory_path_Strat_2_drive2 = '/Users/tristan.venot/Desktop/Experimentation_dataset/Batch2/PSD/PSD_04/_2_/Drive2/*'
directory_path_Strat_3_drive2 = '/Users/tristan.venot/Desktop/Experimentation_dataset/Batch2/PSD/PSD_04/_3_/Drive2/*'
...
Strat_1_DRIVE_CLUSTER = np.array([Strat_1_drive1_mat,Strat_1_drive2_mat]).mean(0)
Strat_2_DRIVE_CLUSTER = np.array([Strat_2_drive1_mat,Strat_2_drive2_mat]).mean(0)
Strat_3_DRIVE_CLUSTER = np.array([Strat_3_drive1_mat,Strat_3_drive2_mat]).mean(0)

Strat_1_DRIVE_CLUSTER = Strat_1_drive2_mat
Strat_2_DRIVE_CLUSTER = Strat_2_drive2_mat
Strat_3_DRIVE_CLUSTER = Strat_3_drive2_mat
...
X = Strat_1_DRIVE_CLUSTER[:,2,:]
T_obs_2, clusters_2, p_values_2, _ = mne.stats.permutation_cluster_1samp_test(X, threshold=thresh, n_permutations=1024, tail=0,adjacency =adjacency ,n_jobs =-1 )
print(clusters_2)
#
print(p_values_2)
T = np.zeros(64)
...
print(List_Clusters_relevant)
for k in range(len(List_Clusters_relevant)):
    for cluster in range(len(List_Clusters_relevant[k])):
        print(np.array(electrodes)[List_Clusters_relevant[k][cluster]])
        T[List_Clusters_relevant[k][cluster]]=1
T_obs_2 = T*T_obs_2


X = Strat_1_DRIVE_CLUSTER[:,3,:]
T_obs_3, clusters_2, p_values_2, _ = mne.stats.permutation_cluster_1samp_test(X, threshold=thresh, n_permutations=1024, tail=0,adjacency =adjacency  ,n_jobs =-1)
T = np.zeros(64)
List_Clusters_relevant = []
for i in range(len(p_values_2)):
    if p_values_2[i]<0.05:
...
X = Strat_1_DRIVE_CLUSTER[:,4,:]
T_obs_4, clusters_2, p_values_2, _ = mne.stats.permutation_cluster_1samp_test(X,threshold=thresh, n_permutations=1024, tail=0,adjacency =adjacency,n_jobs =-1  )
T = np.zeros(64)
List_Clusters_relevant = []
for i in range(len(p_values_2)):
    if p_values_2[i]<0.05:
...
np.savetxt("Cluster_St_1_Dr2.csv", T_obs_3,
              delimiter = ",")
topo_plot(np.array([T_obs_3,T_obs_3]).T,0,electrodes,fres,fs,'Wilcoxon',-6,6)
...
# topo_plot(np.array([T_obs_2,T_obs_2]).T,0,electrodes,fres,fs,'Wilcoxon',-4,4,axes =axs[0])
np.savetxt("Cluster_St_2_Dr2.csv", T_obs_3,
              delimiter = ",")
topo_plot(np.array([T_obs_3,T_obs_3]).T,0,electrodes,fres,fs,'Wilcoxon',-6,6)
...
# topo_plot(np.array([T_obs_2,T_obs_2]).T,0,electrodes,fres,fs,'Wilcoxon',-4,4,axes =axs[0])
np.savetxt("Cluster_St_3_Dr2.csv", T_obs_3,
              delimiter = ",")
topo_plot(np.array([T_obs_3,T_obs_3]).T,0,electrodes,fres,fs,'Wilcoxon',-6,6)
...
Strat_1_cali_mat = 100*Strat_1_cali_mat.mean(0)
Strat_2_cali_mat = 100*Strat_2_cali_mat.mean(0)
Strat_3_cali_mat = 100*Strat_3_cali_mat.mean(0)

Strat_1_drive1_mat = 100*Strat_1_drive1_mat.mean(0)
Strat_2_drive1_mat = 100*Strat_2_drive1_mat.mean(0)
Strat_3_drive1_mat = 100*Strat_3_drive1_mat.mean(0)
...
topo_plot(np.array([St_1[3,:],St_1[3,:]]).T,0,electrodes,fres,fs,'Wilcoxon',-30,30)


fig, axs = plt.subplots()

topo_plot(np.array([St_2[3,:],St_2[3,:]]).T,0,electrodes,fres,fs,'Wilcoxon',-30,30)


fig, axs = plt.subplots()

topo_plot(np.array([St_3[3,:],St_3[3,:]]).T,0,electrodes,fres,fs,'Wilcoxon',-30,30)




zbin = np.linspace(0.0, 15.0, 25)
...
#    fig, axs = plt.subplots(1,3)
    ...
    for i in range(2,5):
        ...
        print((St_1<(-zbin[l]))[i,:].shape)
        significant_coords = pos[(St_1<(-zbin[l]))[i,:]]
        n_significant = significant_coords.shape[0]
        # Compute the pairwise Euclidean distances between significant electrodes
        distances = np.zeros((n_significant, n_significant))
        for k in range(n_significant):
    ...
    for i in range(2,5):
        ...
        significant_coords = pos[(St_2<(-zbin[l]))[i,:]]
        n_significant = significant_coords.shape[0]
        # Compute the pairwise Euclidean distances between significant electrodes
        distances = np.zeros((n_significant, n_significant))
        for k in range(n_significant):
    ...
    for i in range(2,5):
        ...
        significant_coords = pos[(St_3<(-zbin[l]))[i,:]]
        n_significant = significant_coords.shape[0]
        # Compute the pairwise Euclidean distances between significant electrodes
        distances = np.zeros((n_significant, n_significant))
        for k in range(n_significant):
    ...
    ALPHA.append([clusters_1[0],clusters_2[0],clusters_3[0]])
    BETA_Low.append([clusters_1[1],clusters_2[1],clusters_3[1]])
    BETA_High.append([clusters_1[2],clusters_2[2],clusters_3[2]])

beta_low = np.array(BETA_Low)
fig, axs = plt.subplots()
axs.set_title('Low Beta')
axs.plot(zbin,beta_low[:,1],'g-o',label = 'Strat 1')
axs.plot(zbin,beta_low[:,2],'r-o',label = 'Strat 2')
axs.plot(zbin,beta_low[:,0],'b-o',label = 'Strat 3')
...
data = {'Strategy 1': a, 'Strategy 2': c, 'Strategy 3': b}
...
sns.barplot(data=df, palette=['#2ca02c', '#d62728', '#1f77b4'],estimator = 'mean',width = .5)
plt.yticks(fontsize=20)
plt.xticks(fontsize=20)
